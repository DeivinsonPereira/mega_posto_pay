...
import 'package:megga_posto_mobile/model/cash_withdrawal_post_model.dart';
...
class RecordCashWithdrawal {
  ...
  Future<void> record() async {
    try {
      Uri uri = Uri.parse(Endpoints.sangria());

      CashWithdrawalPostModel bodyRequest = CashWithdrawalPostModel(
          ...
          idconta_sangria: _configController.dataPos.idContaSangria ?? 0,
  ...
  void _handleSuccess() {
    const CustomCherrySuccess(message: 'Sangria registrada com sucesso.')
        .show(Get.context!);
    QuantityBack.back(2);
  }
  ...
  void _handleError(String message) {
    ...
    const CustomCherryError(message: 'Erro ao registrar a sangria.')
        .show(Get.context!);
    Get.back();
  }
}
